"use client";
import { NavigationBar } from "@/components/Navbar";
import { Header, Paragraph } from "@/components/Typography";
import { FeaturePlaceholder } from "@/components/FeaturePlaceholder";
import { AboutSection, Benefit, Testimonials } from "@/components/About";
import { benefitsData, testimonials } from "@data";
import { navItemsUnloggedIn } from "@data";
import { useAuth } from "@/context/AuthContext";

export default function Publication(){
    return(
        <>
        {/* PAGENYA SEBENARNYA RESEARCH */}
        <NavigationBar navItems={navItemsUnloggedIn} current_item="About" login={true}/>
        <section className="py-32">
        <Header>Statistical Learning-Based 
            <br/>Analysis of Human Driver Model Parameters</Header>
            <div className="flex justify-center py-12 min-h-screen">
            <div className="w-[794px] h-[1123px] p-10 overflow-y-auto">
            {/* Header Author */}
            <div className="p-2 border-[#A7C4EC] border-t flex justify-between items-center text-sm text-gray-600 mb-4">
                
                {/* Kiri: Avatar + Nama & Tanggal */}
                <div className="flex items-center gap-3">
                <div className="bg-[#E6C798] rounded-full w-10 h-10" />
                <div className="flex flex-col">
                    <div className="font-medium text-[#F5F8FA]">Adinda Putri</div>
                    <div className="text-[#A7C4EC]">26 April 2025</div>
                </div>
                {/* <div><button className="h-10 bg-[#A7C4EC] rounded-full text-white p-5">Fund this research</button></div> */}
                </div>

                {/* Kanan: Stats */}
                <div className="flex gap-4 items-center">
                <span>101</span>
                <span>↑</span>
                <span>|</span>
                <span>↓</span>
                <span></span>
                </div>
            </div>

            <img src="abstract.png"/>
        </div>
        </div>
        </section>
        </>
    )
}