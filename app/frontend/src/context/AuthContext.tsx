...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const savedResearch = localStorage.getItem('all_research');
    ...
    if (savedResearch) {
      setAllResearch(JSON.parse(savedResearch));
    }
  }, []);

  // Save to localStorage whenever data changes
  useEffect(() => {
    localStorage.setItem('auth_status', isLoggedIn.toString());
    if (user) {
  ...
  useEffect(() => {
    localStorage.setItem('all_research', JSON.stringify(allResearch));
  }, [allResearch]);
