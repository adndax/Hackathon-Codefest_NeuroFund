...
export const BlueButton = ({ children, className, onClick}: {children: React.ReactNode, className?: string, onClick?: () => void}) => {
  return (
  <button
    onClick={onClick} // Teruskan onClick ke elemen button
    className="px-12 py-3 mt-15 font-inter font-semibold text-[20px] rounded-md bg-[#A7C4EC] bg-opacity-85 text-background text-sm cursor-pointer hover:-translate-y-1 transform transition duration-200 hover:shadow-md"
  >
    {children}
  </button>
  )
